...
float visibility = 18.0; //water visibility
...
nCoord = coords * (scale * 0.025) + windDir * timer * (windSpeed*0.03);
...
nCoord = coords * (scale * 0.2) + windDir * timer * (windSpeed*0.1)-(col.xy/col.zz)*bend;
...
nCoord = coords * (scale* 0.8) + windDir * timer * (windSpeed*1.0)-(col.xy/col.zz)*bend;
col += texture2D(tex, nCoord + vec2(-timer*0.06,+timer*0.08)).rgb*0.15;
nCoord = coords * (scale * 1.0) + windDir * timer * (windSpeed*1.3)-(col.xy/col.zz)*bend;
...
void main() {
    ...
    vec3 diffuse = texture2D(TextureSampler,gl_TexCoord[0].st*4.0).rgb;      

	float NdotL = max(dot(normal,LV),0.0);


    float sunFade = clamp((LightVec.z+50.0)/300.0,0.0,1.0);
    ...
    vec3 skyLight = mix(vec3(1.0,0.5,0.0)*0.05, vec3(0.2,0.5,1.0), clamp(1.0-exp(-(LightVec.z/500.0)*sunext),0.0,1.0));
    skyLight *= skyBright;
    ...
    float shorecut = smoothstep(-0.001,-0.000,depth);
    
    float shorewetcut = smoothstep(-0.2,-0.000,depth+0.05);
    ...
    fogdarkness = mix(1.0,clamp((eyePos.z+fogdarkness)/fogdarkness,0.0,1.0),shorecut);
    ...
    vec3 fogging = mix((diffuse*0.2+0.8)*mix(vec3(1.1,0.8,0.48),vec3(1.0,0.75,0.5)*0.8,shorewetcut)*color, waterColor*fogdarkness, clamp(fog/waterext,0.0,1.0));//adding water color fog
    
    //vec3 light = ((diffuse*0.2+0.8)*vec3(1.0,0.85,0.5)*(NdotL+skyLight+groundLight));
