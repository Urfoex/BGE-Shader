...
void main()
{
	...
	float width = 0.004 * widthf;    // width = how wide of a sample to use (is repeated 32 times below (8 times vertically, 4 times for each of those vertically)
	...
	gl_FragColor = sum*(strength / 3.0) * texture2D(bgl_RenderedTexture, texcoord); // Usually sum*0.08; 0.08 < is how bright the bloom effect appears on the screen; should probably be around 0.32
}